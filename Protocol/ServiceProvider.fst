...
let serviceprovider me pubk privk browser idp pubkidp = 
  ...
  | Post (sp, params, cookies) -> 
      let msg = getStrInTupleList params "SamlMessage" in
      let sigIDP = getStrInTupleList params "Signature" in
      if VerifySignature idp pubkidp msg sigIDP
      then
        (assert(Log idp msg);
          let ak =  GenerateNonce sp in
          let resp = Response 200 (getContent "Loggedin") [("AuthenticationKey", ak)] in
          Send resp) (*8*)
